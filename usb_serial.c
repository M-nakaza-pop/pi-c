...
***************************************************************************/
int		fd;
char	sbuff[]={"0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ\0"};
char	buff[100];
FILE	*fp;
/***************************************************************************
*
...
int		c=0,i=0;
	...
	for(;;){
		fd= serialOpen("/dev/ttyUSB0",2000000);  //115200 230400 460800
		if(fd< 0){
	...
	if(wiringPiSetupGpio() < 0){
		printf("ERROR:WiringPi initi}alize");
	}
	pinMode(GPIO18,OUTPUT);
	digitalWrite(GPIO18, 0);

	fp=	fopen("rcvlog.txt","w");
	if(fp== NULL){
		puts("file Err\n");
	}
